 ...
 * @author sabrinasouto
 *
 * Implements a Set of 8 sets, where these sets
 * can be a single set, a set of 2, 3, 4, 5, 6, 7  
 * or 8 elements, since they implements ElementIF.
 * In this case, the parent is set1 and it has 7 
 * children (set2, 3, 4, 5, 6, 7 and 8).
 */
public class CompositeTree8<T> extends CompositeTree7<T> {
	
	private static final long serialVersionUID = 1L;
	protected final int id = TreeFactory.TREE_ID++;
	protected Tree<T> set8;
	protected Tree<T> child7;
	
	protected CompositeTree8(){}
			
	protected CompositeTree8(Tree<T> set1, 
				   Tree<T> set2, 
				   Tree<T> set3, 
				   Tree<T> set4,
				   Tree<T> set5, 
...
//		addChild(set8);
//		this.child7 = set8;
//		processInteractions(parent, child7);
		this.parent = set1;
